<template>
  <ClientOnly>
    <div class="demo">
      ...
      <div v-if="isExpanded" class="highlight-closed" @click="codeclick">隐藏源代码</div>
    </div>
  </ClientOnly>
</template>
<script setup>
...
const codeclick = () => {
  ...
  isExpanded.value
    ? highlightRef.value.animate([{ height: highlightRef.value.scrollHeight + "px" }], {
        duration: 500,
        fill: "forwards",
      })
    : highlightRef.value.animate([{ height: 0 }], { duration: 500, fill: "forwards" });
};
onMounted(() => {
